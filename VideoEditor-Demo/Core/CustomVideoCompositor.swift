...
////               let underlayImageBuffer = request.sourceFrame(byTrackID: request.sourceTrackIDs.last!.int32Value) {
////                let range = request.videoCompositionInstruction.timeRange
////                let transitionProgress = range.fraction(of: request.compositionTime)
////               // let smoothProgress = CGFloat(simd_smoothstep(0, 1, Float(transitionProgress)))
////                let underlayImage = CIImage(cvPixelBuffer: underlayImageBuffer)
////                    .fitted(in: renderContext)
//////                    {
//////                        $0.settingScale(x: smoothProgress, y: smoothProgress)
//////                            .settingRotation(.pi / 4 * (1 - smoothProgress))
//////                    }
...
//            .fitted(forRenderIn: renderContext,
//                           uvTransform: { $0
